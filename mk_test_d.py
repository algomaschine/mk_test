...
# this bit is written by Eduard Samokhvalov
def mk_test_call( y,m,d, min_prices ):		
	trend, H, p, z = mk_test(min_prices)
	#print(min_prices)
	...
	if (H==True):
		...
		h="0"
	print (y,",",m,",",d,",",trend,",",h,"," ,p, ",",z)
	
	#debug
	#print(y,m,d,h,min_prices)
	
def process():

	charts = sys.argv[1] # number of parts to split into
	with open(charts) as f:
		lines = f.readlines()

	print("Year, Month, Day, Trend, H, P, Z") # our data header
	
	first_line=True
	#prev values of year,month,day,hour,minute
	py=-1
	pm=-1
	pd=-1
	
	min_prices = [] # reset it every time we do mk_test
	
	for line in lines:
		if (first_line): 
			first_line=False
			continue
		else: # not the first line that contains headers
			
			# 2007 , 01 , 01 , 20 , 00 , 93.950000 , 93.950000 , 93.920000 , 93.930000
			l = line.strip() # hz zachem
			s = l.split(',')
		
			# add numeric conversions here
			y = int(s[0])
			m = int(s[1])
			d = int(s[2])
			close = float(s[8])
			...
			if (calc):
				if len(min_prices)>0:
					mk_test_call(py,pm,pd,  min_prices)
				min_prices=[]
				min_prices.append(close)
				py=y
				pm=m
...
if __name__== "__main__":
  '''
  This code takes the file BTCUSD-1min.csv, which has BTCUSD quotes with resolution of 1 minute. Then it splits it into periods of 1 day, calculating MK Test for each daily period.
  MK test is basically a method that takes a data series and outputs one number (Z) representing volatility. The sign represents trend's direction (up or down). 
  The bigger is of the number - the smoother the trend is, the smaller the number is - more choppy is the trend (we call it a 'whipsaw' in trading jargon).

  The utility of that is to use trend data for visualizing trend stats by periods. 
  For instance we can take 10 years of 1min data for EURUSD and calculate what periods are statistically most volatile over a year (be it months, or days of weeks - what so ever).
  Here's how you can use the output of that code to visualize trends for every day of every month in Excel
  https://yadi.sk/d/Tf9zeppHMuu6ow
  
  Example: python mk_test_d.py BTCUSD-1min.csv 
  '''
  
  # let's run it 10 times and calculate an average
  # OK, we may need like 100 times for prescision, but let's leave it like that for now, cause it takes roughly 10 min already :)
  run_times = 10
  result = 0
  ...
  print('Time taken in seconds:', result) # see the difference after converting the code to C/C++